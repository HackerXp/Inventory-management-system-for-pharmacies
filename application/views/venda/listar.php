                ...
                <div class="panel-container show">
                    <div class="panel-content">
                        <table id="dt-basic-example" class="table table-bordered w-100 text-center">
                            ...
                            <?php
                            $cont_x=0;
                            foreach ($vendas as $res):
                                $cont_x+=1;
                                ?>
                                <tr>
                                    <td> <?= $cont_x?> </td>
                                    <td><?= $res->qtde_prod?></td>
                                    <td><?= number_format($res->valor_total,2)?></td>
                                    <td> <?= nome_utilizador($res->user)?> </td>
                                    <td> <?= $res->comprador?> </td>
                                    <td><?= date("d-m-Y",strtotime($res->data_criacao))?></td>
                                    <td class="text-center">
                                        ...
                                        foreach ($menus as $menu) {
                                            ...
                                            foreach ($sub_menu as $sub) {
                                                ...
                                                if ($sub->modal != 1) {
                                                    ?>
                                                    <a target="_blank" href="<?= site_url($sub->controlador.'/'.$sub->accao.'/' . $res->id) ?>"
                                                    ...
                                                    <a data-toggle="modal"
                                                       ...
                                                       titulo="Desejas realmente elminar esta venda?"
                                                       cod="<?= $res->id ?>" url="<?= site_url('venda/delete') ?>"
